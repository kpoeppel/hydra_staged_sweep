...
def test_resolve_circular_dependency():
    p0 = SweepPoint(
        index=0, parameters={"stage": "A", "ref": "${sibling.B.x}"}, group_path=(0, 0), stage_path=(False, False)
    )
    p1 = SweepPoint(
...
def test_build_dag_warning(caplog):
    ...
    p = SweepPoint(index=0, parameters={"ref": "${sibling.missing.x}"}, group_path=(0,), stage_path=(True,))
